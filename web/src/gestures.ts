...
export interface GestureOptions {
  deadzone?: number;
  // ZOOM TEMPORARILY DISABLED — leg-raise threshold felt too close to the board edge.
  ...
  panInertiaMs?: number;
  // zoomInertiaMs?: number;  // ZOOM TEMPORARILY DISABLED
}
...
// ZOOM TEMPORARILY DISABLED — BobFrame only used by bob-amplitude rolling window.
// interface BobFrame {
//   t: number;
//   total: number;
// }
...
export class GestureInterpreter extends EventTarget {
  ...
  // ZOOM TEMPORARILY DISABLED — bob history only fed zoom speed scaling.
  // private bobHistory: BobFrame[] = [];

  /** Smoothed pan velocity updated by `tick()`. */
  private velPanX = 0;
  private velPanY = 0;
  // private velZoom = 0;  // ZOOM TEMPORARILY DISABLED
  ...
  onSample(sample: BoardSample): void {
    ...
    // this.updateBobHistory(sample, nowMs);  // ZOOM TEMPORARILY DISABLED
  ...
  resetRezero(): void {
    ...
    this.velPanX = 0;
    this.velPanY = 0;
    // this.velZoom = 0;  // ZOOM TEMPORARILY DISABLED
  ...
  tick(dt: number): PanZoomCommand {
    ...
    // Snap sub-threshold pan velocity to zero to eliminate perpetual micro-drift.
    ...
    return {
      ...
      zoom: 0, // ZOOM TEMPORARILY DISABLED
      mode: target.mode,
    };
  }
  ...
  private classify(sample: BoardSample): Mode {
    ...
    // ZOOM TEMPORARILY DISABLED — leg-raise felt too close to the board edge in practice.
  ...
  // ---------------------------------------------------------------------------
  // ZOOM TEMPORARILY DISABLED — Bob amplitude (rolling-window peak-to-trough on total weight)
  // Re-enable when zoom interaction design is revisited.
  // ---------------------------------------------------------------------------
  ...
  private computeCommand(sample: BoardSample): PanZoomCommand {
    ...
    // ZOOM TEMPORARILY DISABLED — ZOOM_IN / ZOOM_OUT branches unreachable while
    // classify() is restricted to ABSENT/PAN. Re-enable with new zoom design.
    ...
    // Fallback (should not be reached while zoom is disabled)
    return { ...ZERO_COMMAND };
  }
}
